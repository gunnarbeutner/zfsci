...
if ! rinse --config ../misc/rinse/rinse.conf --pkgs-dir ../misc/rinse/ \
		--directory $BUILDDIR --distribution centos-5 --arch amd64; then
	echo "rinse failed"
	exit 1
fi
...
wget -O $BUILDDIR/epel-release-5-4.noarch.rpm http://download.fedoraproject.org/pub/epel/5/i386/epel-release-5-4.noarch.rpm
chroot $BUILDDIR rpm -i epel-release-5-4.noarch.rpm
rm -f $BUILDDIR/epel-release-5-4.noarch.rpm
...
tar cfzC /tmp/centos.tar.gz $BUILDDIR .
mv /tmp/centos.tar.gz $IMAGEFILE
